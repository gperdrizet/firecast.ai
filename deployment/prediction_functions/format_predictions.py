...
def format_predictions(data: 'numpy.ndarray', lat_lon_bins: list) -> 'DataFrame':
    ...
    date_list = pd.date_range(datetime.today(), periods=8).tolist()
    print(f'Dates {date_list}')
    print(f'Date list length {len(date_list)}')
    print(f'Predictions length {len(data)}')
